...
try:
    with engine.connect() as conn:
        ...
        # Migraciones de salvaguarda para la tabla metricas en PostgreSQL
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS boton_sin_boton INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_cosecha INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_estrella INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_rayando INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_garbanzo INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_alberja INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_arroz INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_largo_sin_boton INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_cosecha INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_estrella INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_rayando INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_garbanzo INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_alberja INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_arroz INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_medio_sin_boton INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_cosecha INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_estrella INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_rayando INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_garbanzo INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_alberja INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_arroz INTEGER DEFAULT 0;"))
        conn.execute(text("ALTER TABLE metricas ADD COLUMN IF NOT EXISTS tallo_corto_sin_boton INTEGER DEFAULT 0;"))
        conn.commit()
    print("[INFO] Columnas validadas e integradas en base de datos.")
except Exception as e:
    print(f"[WARNING] Fallo no crítico en la migración de coordenadas de camas: {e}")

# Poblar automáticamente datos históricos continuos para la presentación comercial si la base de datos está vacía
def sembrar_datos_demo_si_esta_vacio():
    try:
        ...
        print(f"[WARNING] Error sembrando datos demo: {err}")
